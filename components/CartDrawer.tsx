    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
      <div className="animate-float-up flex w-full max-w-2xl flex-col rounded-t-[24px] bg-background max-h-[90vh]">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div>
            <h2 className="font-display text-[19px] font-semibold">Coșul meu</h2>
            <span className="brand-rule mt-1" />
          </div>
          <button
            onClick={onClose}
            aria-label="Închide"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-surface border border-border text-ink"
          >
            ✕
          </button>
        </div>
          ...
          {items.length === 0 ? (
            ...
            <ul className="space-y-2.5">
              {items.map((item) => (
                <li
                  key={item.line_id}
                  className="flex items-start justify-between gap-3 rounded-[16px] border border-border bg-surface p-3.5"
                >
                  ...
                  <div className="flex flex-shrink-0 items-center gap-3 rounded-pill border border-border bg-background px-3 py-1.5">
                    <button
                      onClick={() => onUpdateQuantity(item.line_id, item.quantity - 1)}
                      aria-label="Scade cantitatea"
                      className="w-4 text-base font-semibold text-ink"
                    >
                      −
                    </button>
        ...
        {items.length > 0 && (
          <div className="border-t border-border px-5 py-4 space-y-3.5">
            ...
            <button
              ...
              className="w-full rounded-pill bg-primary px-5 py-3.5 text-[15px] font-semibold text-white transition-transform active:scale-[0.98] hover:bg-primary-dark"
            >
              ARATĂ CHELNERULUI
            </button>
            <button
              onClick={onClear}
              className="w-full rounded-pill border border-border px-5 py-3 text-[13.5px] font-medium text-ink-soft transition-transform active:scale-[0.98]"
            >
              GOLEȘTE COȘUL
            </button>
          </div>
